  ...
  describe "for authorized user " do
    ...
    describe "with incorrect params" do
      describe "POST /ads" do
        it "create a new ad with existing сar" do
          expect{
            post '/api/v1/ads', params: {car: {engine_capacity: car.engine_capacity, creation_year: car.creation_year, model_id: model.id}, ad: {price: 6000, description: "Test desc!", user_id: user.id}}, headers: @auth_params
          }.to change {Ad.count}.from(0).to(1)
          expect(response).to have_http_status(:created)
          expect(response_body["description"]).to eq("Test desc!")
        end

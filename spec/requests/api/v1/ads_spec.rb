  ...
  describe "for authorized user " do
    ...
    describe "POST /ads" do
      it "create a new ad" do
        expect{
          post '/api/v1/ads', params: {car: {engine_capacity: 1.9, creation_year: 1996, model_id: model.id}, ad: {price: 6000, description: "Test desc!", user_id: user.id}}, headers: @auth_params
    ...
    describe "PATCH /ads/:id" do
      let!(:ad){FactoryBot.create(:ad, description: 'Test desc N1!', price: 4000, car: car, user: user) }
      it "update the ad" do
        patch "/api/v1/ads/#{ad.id}", params: {car: {engine_capacity: 2.4, creation_year: 1996, model_id: model.id}, ad: {price: 6000, description: "Patch success!", user_id: user.id}}, headers: @auth_params
        expect(response).to have_http_status(:success)
